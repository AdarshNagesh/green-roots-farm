...
export default function AdminPage() {
  ...
  async function loadAll() {
    ...
    setStats({
      ...
      customers: 0, // updated below
    })
    loadCustomers()
  }

  // Fetch customers via service role API route to bypass RLS
  async function loadCustomers() {
    // Uses the Supabase admin client (service role) on the server-side API
  ...
  async function saveProduct() {
    ...
    setForm(BLANK); setEditing(false); loadAll()
  }
  ...
  async function updateOrderStatus(order, newStatus) {
    ...
    await sendOrderNotifications({ ...order, user_email: order.user_email, customer_name: order.customer_name, address: order.address, phone: order.phone }, newStatus)

    showToast(`Status updated → ${newStatus}. Customer notified via email & SMS!`)
    setUpdatingOrder(null)
  }

  function startEdit(prod) {
    setForm({ ...prod, price:String(prod.price) }); setEditing(true); setTab('products')
    ...
    <>
      ...
      <main style={{ maxWidth:1120, margin:'0 auto', padding:'28px 20px' }}>
        <div style={{ marginBottom:28 }}>
          ...
          <div style={{ fontSize:13, color:'var(--muted)' }}>Manage produce, fulfil orders, and view your customers.</div>
        </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 360px', gap:24, alignItems:'start' }}>
                      ...
                      <div key={p.id} className="card" style={{ padding:'12px 16px', display:'flex', alignItems:'center', gap:12 }}>
                        ...
                        <div style={{ display:'flex', gap:6 }}>
                          <button onClick={() => startEdit(p)}
                            style={{ padding:'5px 12px', border:'1px solid var(--border)', borderRadius:7, background:'transparent', cursor:'pointer', fontSize:12 }}>Edit</button>
                          <button onClick={() => deleteProduct(p.id)}
                            style={{ padding:'5px 12px', border:'1px solid var(--border)', borderRadius:7, background:'transparent', cursor:'pointer', fontSize:12, color:'var(--red)' }}>Delete</button>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div className="card" style={{ padding:22, position:'sticky', top:80 }}>
              ...
              <div style={{ marginBottom:14 }}>
                <div style={{ fontSize:12, color:'var(--muted)', fontWeight:500, marginBottom:7 }}>Emoji</div>
                <div style={{ display:'flex', flexWrap:'wrap', gap:3 }}>
                  {EMOJIS.map(e => (
                    <button key={e}
                      style={{ fontSize:20, padding:5, border:`2px solid ${form.emoji===e?'var(--green)':'transparent'}`,
                        borderRadius:7, background:form.emoji===e?'var(--green-pale)':'none', cursor:'pointer' }}
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:12 }}>
                <div>
                  ...
                  <input className="inp" type="number" min="0" step="0.5" value={form.price} onChange={e => set('price', e.target.value)} />
                </div>
                <div>
              ...
              <div style={{ display:'flex', gap:8 }}>
                ...
                {editing && <button className="btn-o" style={{ padding:'10px 14px' }} onClick={() => { setForm(BLANK); setEditing(false) }}>Cancel</button>}
              </div>
                      ...
                      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:12 }}>
                        <div>
                          ...
                          <div style={{ fontSize:12, color:'var(--muted)' }}>
                            ...
                            {o.payment_status==='paid' && <span style={{ color:'var(--green)', fontWeight:600 }}> ✓ Verified</span>}
                          </div>
                        </div>
                        <div style={{ textAlign:'right' }}>
                          ...
                          {updatingOrder===o.id && <div style={{ fontSize:11, color:'var(--muted)', marginTop:4 }}>Notifying customer…</div>}
                        </div>
                      </div>
